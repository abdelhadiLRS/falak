...
// Increase maximum event listeners to prevent memory leak warnings in proxy setup
...
const verifySignature = (req: any, res: any) => {
...
const attachCookies = (proxyReq: any, req: any) => {
  if (req.headers.cookie) {
    proxyReq.setHeader('Cookie', req.headers.cookie);
  }
};

const attachSignatureHeaders = (proxyReq: any, req: any) => {
...
const apiProxy = createProxyMiddleware<NextApiRequest, NextApiResponse>({
  ...
  on: {
    ...
    error: (err, req, res) => {
      if ('status' in res && typeof (res as NextApiResponse).status === 'function') {
        (res as NextApiResponse).status(500).json({ error: ERROR_MESSAGES.PROXY_ERROR, message: err.message });
      } else {
...
export const config = {
  api: {
    bodyParser: {
      sizeLimit: '8mb', // ✅ إسناد قيمة نصية مباشرة لتجاوز خطأ الـ Static Analysis
    },
  },
};
