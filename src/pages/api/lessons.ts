...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (method === 'GET') {
    const { category, teacher, search, limit = '20', page = '1' } = req.query;
    const take = parseInt(limit as string, 10);
    const skip = (parseInt(page as string, 10) - 1) * take;

    const where: any = {};
    if (category) where.category = category as string;
    if (teacher) where.teacher = teacher as string;
    if (search) {
      where.OR = [
        { title: { contains: search as string } },
        { description: { contains: search as string } },
        { teacher: { contains: search as string } },
      ];
    }
  ...
  if (method === 'POST') {
    ...
    const { title, category, teacher, videoUrl, audioUrl, pdfUrl, description } = req.body;
    ...
    try {
      const item = await prisma.lesson.create({
        data: { title, category, teacher, videoUrl, audioUrl, pdfUrl, description },
      });
