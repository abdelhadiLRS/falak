...
export const getFullDayName = (day: Date, locale: string): string => {
  try {
    const fullLocale = getLangFullLocale(locale) || locale;
    return day.toLocaleDateString(fullLocale, { weekday: 'long', timeZone: 'UTC' });
  } catch (error) {
...
export const getShortDayName = (day: Date, locale: string): string => {
  try {
    const fullLocale = getLangFullLocale(locale) || locale;
    return day.toLocaleDateString(fullLocale, { weekday: 'narrow', timeZone: 'UTC' });
  } catch (error) {
    return day.toLocaleDateString('en-US', { weekday: 'narrow', timeZone: 'UTC' });
  }
};
...
export const getFullMonthName = (month: Date, locale: string): string => {
  try {
    const fullLocale = getLangFullLocale(locale) || locale;
    return month.toLocaleDateString(fullLocale, { month: 'long', timeZone: 'UTC' });
  } catch (error) {
    return month.toLocaleDateString('en-US', { month: 'long', timeZone: 'UTC' });
  }
};
